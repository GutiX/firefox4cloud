...
//-------	SYNONYMS ACTIONS	-----------
/*var synonymsPageWorker = pageWorkers.Page({
...
synonymsPageWorker.port.on("synonymsAdaptor", function(settings) {
	console.log("### synonymsAdaptor... " + settings);
});
pageMod.PageMod({
  include: "*",
  contentStyleFile: self.data.url("css/content.js"),
  attachTo: ["existing", "top"]
});*/

var synonymsPageMod = pageMod.PageMod({
  include: "*",
  contentScriptFile: [self.data.url('js/jquery-1.9.1.min.js'), self.data.url('js/synonyms.js')],
	...
	var preferencesRequest = Request({
		...
		onComplete: function(response) {
			if (response.status == 200) {
				...
				synonymsPageMod.port.emit("synonymsProcess", response.json);
			} else {
...
function onSynonymsChange(){
	...
	synonymsPageMod.port.emit("synonyms", { action : "enable synonyms", status : simplePrefs.prefs.synonyms });
}

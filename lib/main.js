...
pageMod.PageMod({
  ...
  onAttach: function(worker) {
	...
	worker.port.on("getCurrentZoom", function(zoomReq) {
		console.log("### ZoomReq... " + zoomReq);
		var zoomCoef = 1;
		if(simplePrefs.prefs.magnification != null)
			zoomCoef = simplePrefs.prefs.magnification;
		tabsModeMap[tabs.activeTab.id].port.emit("currentZoom", zoomCoef);
	});
	...
	var preferencesRequest = Request({
		...
		onComplete: function(response) {
			if (response.status == 200) {
				...
				tabsModeMap[tabs.activeTab.id].port.emit("synonymsProcess", error);
			}
		}
	});	
...
function onMagnificationChange() {
	...
	//removeStyle("scale");
	removeStyle("-moz-transform");
	insertStyle("html {-moz-transform: scale(" + simplePrefs.prefs.magnification + ");}");
	insertStyle("html {-moz-transform-origin: 0 0;}");
...
function onBackgroundColorChange() {
	...
	if(background != "")
	{
		...
		insertStyle("body, div, p, input, span, h1, h2, h3, h4, h5, h6, button, a, ul, li, footer, header, talbe, th, tr, td, code, article, nav, section, dl, dt, strong { background-color: " + background + " !important;}");
		insertStyle("a span { background-color: transparent !important;}");
		if(simplePrefs.prefs.foregroundColor == "#000000" && simplePrefs.prefs.backgroundColor == "#FFFFFF")
		{
...
function onForegroundColorChange() {
	...
	if(foreground != "")
	{
		...
		if(simplePrefs.prefs.foregroundColor == "#FFFFFF" || (simplePrefs.prefs.foregroundColor == "#000000" && simplePrefs.prefs.backgroundColor == "#FFFFFF"))
		{
			insertStyle("body{filter: grayscale(100%);}");
		}
		else
